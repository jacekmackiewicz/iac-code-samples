...
- Use WSL on Windows 10, then install on Ubuntu using those instructions:
...
# already in the repository - Create a playbook file with that content, e.g. `your_playbook.yml`
# not needed - Fetch this role `ansible-galaxy install -n -p ./roles Graylog2.graylog-ansible-role`
# not needed - Install role's dependencies `ansible-galaxy install -r roles/Graylog2.graylog-ansible-role/requirements.yml -p ./roles`
- Change the graylog_password_secret and graylog_root_password_sha2 variables in ansible-playbook.yml
- Log in to the VM with your user credentials so ansible won't reject your fingerpting `ssh user@127.0.0.1`
- Apply the playbook to VM `ansible-playbook ansible-playbook.yml -i "VM WAN IP:22," --ask-pass`
...
graylog_server_version:     "3.2" # Optional, if not provided the latest version will be installed
...
- Set up playbook (see example below) and ansible-playbook-dev.yml for Powel specific configuration:
...
- Be sure the domain in inventory file points to VM WAN IP:
  - for dev set https://portal.azure.com/#@powelas.onmicrosoft.com/resource/subscriptions/196e0d0e-0b80-4da5-bbcf-b0ec44cd0eb9/resourceGroups/powel-net-dev-dns/providers/Microsoft.Network/dnszones/poweltest1.com/overview
  - for production contact itsupport@powel.com
- Run the playbook with `ansible-playbook -i inventory_file your_playbook.yml`
- Login to Graylog by opening `https://<host domain address>` in your browser, default username and password is `admin`
